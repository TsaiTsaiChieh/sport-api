    ...
    for (const god of Object.values(gods)) {
      ...
      for (const reward of Object.values(rewardInfo)) { // 獎勵s 有回傳代表有符合首次大神條件
        ...
        if (rewardInfo.length > 0) { // 代表有獎勵
          ...
          await db.User.update({ ingot: purse_self.ingot + issue.reward_value }, { where: { uid: god.uid } });
          /* 存入錢包-END */
          ...
          const [err] = await to(setUserMissionStatus(god.uid, missionStatusUpdateParms, 2));
      ...
      for (const cashflow_mlist of cashflow_mission_list) {
        if (cashflow_mlist.mission_deposit_id) {
          ...
          try {
            ...
            await to(setUserMissionStatus(users_mission.uid, missionStatusUpdateParms, 2));
