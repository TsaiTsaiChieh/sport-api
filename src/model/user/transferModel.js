        ...
        SELECT * FROM(
         ...
         SELECT 0 as ingot, cd.coin, cd.dividend, "打賞此篇文章" as title, "donated" as en_title, NULL as name, NULL as name_ch, NULL as display_name, ta.title as article_title, scheduled 
          ...
          WHERE cd.article_id = ta.article_id
            ...
            AND scheduled BETWEEN :begin AND :end
          UNION
        SELECT 0 as ingot, cb.coin, cb.dividend, "購牌消費" as title, "buy" as en_title, vl.name, vl.name AS name_ch, display_name, NULL as article_title, scheduled
          FROM cashflow_buys cb, view__leagues vl, users u
         WHERE cb.league_id = vl.league_id
           AND cb.uid = u.uid
           AND cb.uid = :uid
           AND cb.status = 1
           AND scheduled BETWEEN :begin AND :end
