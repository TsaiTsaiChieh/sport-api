  ...
  const timerForStatus2 = setInterval(async function() {
    ...
    if (parameter.first === 0) {
      if (realtimeData.Summary.info) {
        if (realtimeData.Summary.changeRight) {
          changeRight = realtimeData.Summary.changeRight;
        }
        ...
        if (realtimeData.Summary.info.away) {
          if (realtimeData.Summary.info.away.Now_lineup) {
            hitterAwayAB =
        ...
        if (realtimeData.Summary.info.home) {
          if (realtimeData.Summary.info.home.Now_lineup !== 0) {
            hitterAwayAB =
...
async function reInitRealtime(betsID, data) {
  return new Promise(async function(resolve, reject) {
    try {
...
async function writeRealtime(betsID, realtimeData, data, baseballParameter) {
  return new Promise(async function(resolve, reject) {
    ...
    let changeRight = baseballParameter.changeRight;
    const homeID =
    ...
    for (let eventCount = eventNow; eventCount < eventEnd; eventCount++) {
      if (changeRight === 0) {
        if (
          data.api.data.competition.season.stage.group.event.details[5]
            .value === 'YES'
        ) {
          const firstHitter = data.api.data.competition.season.stage.group.event.events_incidents.findIndex(
            (ele) => ele.incident_name === 'Batter in the box'
          );
          if (firstHitter !== -1) {
            // 判斷第一位打擊手是否與realtime database一致，不一致則重新 initial
            if (
              data.api.data.competition.season.stage.group.event
                .events_incidents[firstHitter].subparticipant_id !==
              realtimeData.Summary.away.Now_lineup.lineup1.id
            ) {
              await reInitRealtime(betsID, data);
            } else {
              changeRight = 1;
              await database
                .ref(`${sport}/${league}/${betsID}/Summary/changeRight`)
                .set(1);
            }
          }
        } else {
