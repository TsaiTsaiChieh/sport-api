            ...
            for (
              ...
              if (
                ...
                if (halfCount === 0) {
                  ...
                  ref = modules.database.ref(
                    `baseball/MLB/${betsID}/Summary/info/home/`
                  );
                  await ref.set({});
                }
              ...
              if (
                ...
                if (
                  ...
                  for (let i = 1; i < 10; i++) {
                    if (
                      hitterID ===
                      realtimeData.Now_lineup[`lineup${i}`].player_id
                    ) {
                      ref = modules.database.ref(
                        `baseball/MLB/${betsID}/Summary/Now_order`
                      );
                      await ref.set(
                        realtimeData.Now_lineup[`lineup${i}`].order
                      );
                    }
                    // here
                  }
...
async function MLBpbpHistory (parameter) {
  ...
  try {
    ...
    const dataPBP = data;
    const ref = await modules.firestore.collection(`${firestoreName}_PBP`).doc(betsID);
    await ref.set(
      {
        bets_id: betsID,
        radar_id: gameID,
