...
async function ESoccerpbpHistory(parameter) {
  ...
  try {
    ...
    try {
      ...
      console.log('pbpHistory set firebase error');
    }
    ...
    try {
      ...
      console.log('pbpHistory set mysql error');
    }
    try {
      ...
      console.log('pbpHistory set realtime error');
    }
  } catch (error) {
    console.log(
      `Error in pubsub/pbpESoccer pbpHistory functions by DY on ${Date.now()}`,
      error
    );
  }
}
  ...
  try {
    ...
    if (data.results[0]) {
        ...
        if (data.results[0].time_status === '3') {
          if (realtimeData.Summary.status !== 'closed') {
            try {
              ...
              console.log('doPBP set realtime error');
            }
          }
        }
        ...
        if (data.results[0].time_status === '2') {
          if (realtimeData.Summary.status !== 'inprogress') {
            ...
            try {
              ...
              console.log('doPBP set mysql error');
            }
            try {
              await modules.firestore
              ...
              console.log('doPBP set firestore error');
            }
          }
        }
      ...
      let ref = modules.database.ref(
        `esports/eSoccer/${betsID}/Summary/league`
      );
      try {
        await ref.set({
          name: data.results[0].league.name,
        ...
        console.log('doPBP set realtime league error');
      }
      ...
      try {
        ...
        console.log('doPBP set realtime timer error');
      }
      ...
      try {
        ...
        console.log('doPBP set realtime info error');
      }
    }
  } catch (error) {
    console.log(
      `Error in pubsub/pbpESoccer doPBP functions by DY on ${Date.now()}`,
      error
    );
  }
}
