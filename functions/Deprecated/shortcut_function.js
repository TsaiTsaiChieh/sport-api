...
exports.lazyFirebaseAdmin = function(
  certStringOrPath = '',
...
exports.runFileContentsUpload = async function(
  FileSplitArray = [],
  newDocRef,
  firestore
) {
  ...
  try {
    firestore = this.setNoValue(
      firestore,
      this.lazyFirebaseAdmin().firestore()
    );
    ...
    for (const SplitFile of FileSplitArray) {
      // @ts-ignore
      //const contents = await fs.readFile( file, 'utf8' );
      let FileSplitDocRef = await firestore
...
exports.runFileUpload5t = async function(fileUrl = '', firestore) {
  ...
  try {
    ...
    let uploadFileData = {
      endTimestamp: this.jsTimeTOfireBaseTime(
        ShortcutFunction.timestampUTCmsInt(7)
      ) //ShortcutFunction.timestampUTCmsInt( 100 ) //更新存活時間
    };
...
exports.realtimePush = async function(
  pushData = {},
  channelId = 'public',
  subPath1 = 'livePush'
) {
...
exports.getOneMessage = async function(inputJson = {}, messageId2 = '') {
  ...
  try {
    ...
    console.info(
      'getOneMessage   getOneShareFile 888888888888888888\n',
      fireJson
    );
    //{
    ...
    console.info(
      'getOneMessage  getOneShareFile data  999999999999999999 \n',
      data
    );
    //} //if fireJson.success
...
exports.getOneShareFile = async function(
  inputJson = {},
  fileUploadId2 = '',
  needBuffer = false
) {
...
exports.sessionToDecodedIdToken = async (__session = '', firebaseAdmin) => {
  try {
    let decodedIdToken =
      (await firebaseAdmin.auth().verifySessionCookie(__session, true)) || {};

    console.info(
      '__sessionToDecodedIdToken  decodedIdToken   >>>>>>',
      decodedIdToken
    );
    ...
    console.warn(
      '__sessionToDecodedIdToken  Auth - verifySessionCookie false : ',
      error
    );
    return {
      error: error
