...
async function messageGetOne(req, res) {
  ...
  try {
    ...
    let p = inputJson.param2 || inputJson.param1;

    returnJson = await ShortcutFunction.getOneMessage(inputJson, p);
  ...
  try {
    ...
    Snapshot.forEach(
      doc => {
        ...
        //999:軟刪除狀態;-1:管理員刪除(回收),0用戶刪除(回收),1用戶刪除(其他人可以看),2,無設定:正常顯示
...
async function runCreateMessage(inputJson = {}) {
  ...
  try {
    ...
    try {
      newMessage.replyMessage = await ShortcutFunction.getOneMessage(
        undefined,
    ...
    newMessage.fileUploadId =
      returnJson.uploadFileReturnJson.fileUploadId || '';
    ...
    let realtimePushReturnJson = await ShortcutFunction.realtimePush(
      newMessage
    );
    returnJson.success = true;
